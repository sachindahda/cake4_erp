const erp_js = {
    ...
    bindCalculateTotalPrice: function () {
        ...
        if ($(_self.cart_price).val() != '') {
            console.log('inside');
            $(_self.total_amt).val(parseFloat($(_self.cart_price).val()) + parseFloat($(_self.transport_price).val() != '' ? $(_self.transport_price).val() : 0) + parseFloat($(_self.staff_price).val() != '' ? $(_self.staff_price).val() : 0) + parseFloat($(_self.gst_amount).html() != '' ? $(_self.gst_amount).html() : 0))
        }
